    ...
    return (
      <main className="flex-grow relative pt-32 pb-24 px-6 md:px-20 overflow-hidden min-h-screen bg-[#0D0D0D]">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="mb-16">
            <h1 className="font-serif text-4xl md:text-5xl text-[#FF4E00] uppercase tracking-widest mb-4">Archives</h1>
            <p className="font-sans text-sm uppercase tracking-[0.2em] text-[#E0D8D0]/60">Select a chapter to begin your exploration</p>
          </div>
            ...
            {CHAPTERS.map((chapter) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-8 flex flex-col justify-end">
                  ...
                  <h2 className="font-serif text-2xl md:text-3xl text-[#E0D8D0] mb-2 group-hover:text-[#FF4E00] transition-colors">{chapter.title}</h2>
                  <p className="font-sans text-xs text-[#E0D8D0]/70 leading-relaxed">{chapter.blurb}</p>
                </div>
              </button>
            ))}
          </div>
    ...
    <main className="flex-grow relative pt-20 overflow-hidden">
      ...
      <section className="relative w-full px-4 md:px-12 pt-6">
        <div className="relative w-full max-w-[1600px] mx-auto aspect-[16/9] overflow-hidden rounded-2xl bg-[#0e0e0e] border border-white/10">
          <img
            ...
            src={heroSrc}
            alt={storyNode.title}
            onError={() => { if (heroSrc !== FALLBACK_HERO) setHeroSrc(FALLBACK_HERO); }}
          ...
          <button
            type="button"
            onClick={() => { hapticTap(); audioEngine.playButtonClick(); setExploreView('chapters'); }}
      ...
      <section className="px-6 md:px-20 py-16 md:py-20 relative z-10 fade-in-rise delay-300">
        <div className="max-w-4xl mx-auto text-center mb-20 md:mb-24">
          <h2 className="font-serif text-3xl md:text-5xl text-[#E0D8D0] italic font-normal leading-relaxed text-balance opacity-95 mb-6">"{storyNode.title}"</h2>
          <p className="font-sans text-lg md:text-2xl text-[#E0D8D0]/80 font-light leading-relaxed text-balance">{storyNode.text}</p>
        ...
        <div className="max-w-7xl mx-auto pb-24">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-12">
            {storyNode.choices.map((choice, idx) => (
              <button
                key={`${choice}-${idx}`}
                disabled={isLoading}
                onClick={() => { hapticConfirm(); audioEngine.playButtonClick(); onSelectChoice(choice); }}
              ...
              >
                ...
                <span className="text-[10px] font-mono text-[#FF4E00] uppercase tracking-[0.3em]">0{idx + 1} // Intention</span>
                <span className="text-xs uppercase tracking-[0.2em] font-semibold text-[#E0D8D0] group-hover:text-[#FF4E00] transition-all duration-300 relative z-10">{choice}</span>
          ...
          <div className="text-center pt-4">
            {!showCustomInput ? (
              <button
                type="button"
                onClick={() => { hapticTap(); audioEngine.playButtonClick(); setShowCustomInput(true); }}
              ...
              <form onSubmit={handleCustomSubmit} className="max-w-xl mx-auto flex gap-3 mt-4">
                <input
                  ...
                  autoFocus
                />
                <button type="submit" className="px-6 py-2.5 bg-[#FF4E00] text-black font-semibold text-xs uppercase tracking-widest hover:bg-[#ff6d2b] transition-colors cursor-pointer rounded-lg">Send</button>
                <button type="button" onClick={() => { hapticTap(); audioEngine.playButtonClick(); setShowCustomInput(false); }} className="px-3 py-2 text-xs text-[#E0D8D0]/50 hover:text-white">✕</button>
              </form>
            )}
          </div>
        </div>
